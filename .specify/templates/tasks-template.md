...
**Tests**: Tests are REQUIRED by the project constitution. Write them before the corresponding
implementation and verify they fail for the intended reason. Unit tests MUST remain offline; real
model and CUDA checks belong in explicitly marked smoke tests.
...
- [ ] T010 Add cross-platform device-selection and path-handling tests
- [ ] T011 Add OOM, codec, invalid-image, unavailable-model, and unsupported-device failure tests
- [ ] T012 Define generation metadata, local artifact retention, and redacted logging behavior
