...
- [ ] T040 [P] [US1] **BLOCKED on the video runtime decision (2026-09-01).** The voice half is proven — XTTS-v2 produces acceptable Arabic at 0.25x realtime in 2.69 GiB — and the 166-character Arabic split it requires is settled. The video half has no measured constant to declare, so writing this task now would mean inventing capability numbers, which `model_registry.validate_profile` exists to refuse. Implement the composite adapter — one `JointAdapter` whose `generate()` synthesizes speech first, then drives `Wan2.2-S2V-14B` through `diffsynth`'s `WanVideoPipeline` with that audio, returning frames and speech together — in `adapters/wan_s2v.py`. `JointAdapter` describes the **interface**, not the model, so two models behind one `generate()` needs no protocol change. Declare `roles={VIDEO,VOICE,LIP_SYNC}` and `native_capabilities={VIDEO,LIP_SYNC}`, which is the shape `domain._check_roles` already validates. Load each stage and free it before the next: nothing co-resides. Round measured speech duration up to the next legal frame count (**4n+1**) and pad the audio tail; resample to 16 kHz for conditioning only, keeping the TTS rate for delivery
...
- [ ] T091 **[hoisted to Stage 0 — BLOCKED 2026-09-01 on the runtime, not the host]** The spike ran on the RTX 5080 and no configuration completed a single denoising step, so there is still nothing to measure. Six configurations (bf16/int8/int4 x cpu/cuda offload x 448x832/320x576 x 81/33 frames) all pin at ~15.8 GiB VRAM and ~61 W with one CPU core saturated; measured host-to-device bandwidth is 36.8 GB/s pinned, so `diffsynth==2.1.5`'s offload loop is ~350x slower than the link allows. See `research.md`, "Wan2.2-S2V on the RTX 5080: measured, and blocked". This task unblocks when the runtime decision is made, not when hardware is available. Measure the **real supported duration ceiling** on the RTX 5080 and record it as a profile field, in `adapters/wan_s2v.py`. FramePack plus the multi-clip path removes the hard wall H3 had, so there is no vendor number to harvest — the ceiling is whatever stays practical on this card and must come from actual generations. Confirm the 16 fps frame rate and the 4n+1 grid at the same time; run the Wan spike first
- [ ] T092 **[hoisted to Stage 0 — PARTIALLY MEASURED 2026-09-01]** Load-time figures exist: 44 s to load, 11.45 GiB host RSS, 0.0 GiB peak reserved under CPU offload (a near-zero VRAM figure at load is expected when nothing co-resides and proves nothing about generation). Generation-time figures do not exist, because no run reached a denoising step. The working set is **45.77 GiB / 49 files**, not the 42.60 GiB / 15 files this task records from the Hugging Face API — the audio encoder ships its weights three times and only the safetensors copy is loaded. Measure resident host footprint per precision against 64 GB and peak allocator-reserved memory against 13.5 GiB, recording both in the profile, in `tests/integration/test_cuda_smoke.py`. The working set is 42.60 GiB at BF16 with a 30.35 GiB largest component; DiffSynth's disk-offload path takes an explicit `vram_limit`, so measure what that budget actually holds before T040 declares a precision. Run the Wan spike `--stage load` first
